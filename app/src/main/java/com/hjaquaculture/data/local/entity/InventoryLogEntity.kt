...
data class InventoryLogEntity(
    ...
    /*
    `changeType` 和 `refOrderId` 之间有天然的映射关系：

    changeType = PURCHASE_IN → 去 purchase_order 表查 refOrderId
    changeType = SALE_OUT    → 去 sale_order 表查 refOrderId
    changeType = LOSS        → refOrderId 为 null，不需要查
    changeType = ADJUST      → refOrderId 为 null，查 refStocktakingId
     */

    @ColumnInfo(name = "ref_document_id")
    val refDocumentId: Long? = null,
